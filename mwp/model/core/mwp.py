...
class MWP(nn.Module):
    ...
    def forward(
        ...
        if sample or self.solvability_checker is not None:
            outputs.generated = self.language_model.generate_from_logits(outputs.logits)
            # outputs.generated = self.language_model.generate_by_sampling(language_model_output.input_encoding)
